...
| [internal/ray/ray.go](internal/ray/ray.go) | Ray cluster membership for `/health.ray` (v0.3.3). Parses the raylet's own command line for `gcs_address` / session name / resources — no `ray status` shell-out. `gcs_address` is the key the backend groups a tensor-parallel serving unit on. |
